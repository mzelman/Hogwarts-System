...
public class CourseServiceImpl implements CourseService {
    ...
    public List<Student> getAllEnrolledStudents(Long courseId) {
        return studentRepository.findByCoursesId(courseId, Sort.by("lastName")
                .ascending().and(Sort.by("firstName")));
    }

    public List<Student> getNotEnrolledStudents(Long courseId) {
        List<Student> students = (studentRepository.findAll(Sort.by("lastName")
    ...
    public boolean checkCourseCodeDuplicate(Course course) {
        for (int i = 0; i < courseRepository.findAll().size(); i++) {
            if (courseRepository.findAll().get(i).getCode().equals(course.getCode())) {
                return true;
            }
    ...
    public boolean checkCourseSubjectDuplicate(Course course) {
        for (int i = 0; i < courseRepository.findAll().size(); i++) {
            if (courseRepository.findAll().get(i).getSubject().equals(course.getSubject())) {
                return true;
            }
        }
        return false;
